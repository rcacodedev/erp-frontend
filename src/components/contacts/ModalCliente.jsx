...
export default function ModalCliente({
  ...
  const validate = () => {
    const e = {};
    const hasName =
      (form.razon_social && form.razon_social.trim()) ||
      (form.nombre && form.nombre.trim());
    if (!hasName) e.nombre = "Indica Razón social o Nombre.";
  ...
  const payload = useMemo(() => {
    ...
    if (form.rating != null && form.rating !== "")
      cliente.rating = Number(form.rating);
    if (String(form.limite_credito).trim() !== "")
      cliente.limite_credito = Number(form.limite_credito);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={handleChange("cliente_desde")}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  className="w-full border rounded px-3 py-2"
                  value={form.tamano}
                  onChange={handleChange("tamano")}
                  placeholder="Pyme / Enterprise"
                />
              </div>
              <div>
                ...
                />
                {formErr.rating && (
                  <p className="text-xs text-red-600 mt-1">{formErr.rating}</p>
                )}
              </div>
              <div>
                <label className="block text-sm mb-1">
