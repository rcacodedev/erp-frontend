...
export default function Suppliers() {
  ...
  const listQueryPath = useMemo(() => {
    ...
    return `/contacts/suppliers/?${p.toString()}`;
  }, [
    searchDeb,
    activo,
    bloqueado,
  ...
  const createProveedor = async (payload) => {
    try {
      ...
      await http.post(tpath(org.slug, "/contacts/suppliers/"), payload);
      setReloadKey((k) => k + 1);
      setToast({ kind: "success", msg: "Proveedor creado correctamente." });
      setOpenNew(false);
    } catch (err) {
      const detail =
  ...
  // DELETE → /contacts/suppliers/{id}/
  const deleteProveedor = async (row) => {
    try {
      if (!org?.slug) throw new Error("Falta el slug de la organización.");
      ...
      if (!ok) return;
      await http.delete(tpath(org.slug, `/contacts/suppliers/${row.id}/`));
      setReloadKey((k) => k + 1);
      setToast({ kind: "success", msg: "Proveedor eliminado correctamente." });
    } catch (err) {
      const detail =
        err?.response?.data?.detail ||
        err?.response?.data?.message ||
    ...
    try {
      ...
      while (true) {
        ...
        const { data } = await http.get(
          tpath(org.slug, `/contacts/suppliers/?${p.toString()}`)
        );
      ...
      downloadCSV("proveedores.csv", mapped);
    } catch (e) {
      setToast({
        kind: "error",
    ...
    <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Razón social, nombre, email…"
              value={search}
      ...
      <ModalProveedor
        ...
        onSubmit={async (payload) => {
          ...
          await updateProveedor(editRow.id, payload);
        }}
